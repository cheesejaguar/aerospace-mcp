...
class WindPoint(BaseModel):
    ...
    wind_direction_deg: float | None = Field(None, description="Wind direction in degrees")
    ...
    for altitude in altitudes_m:
        ...
        if AMBIANCE_AVAILABLE and model_type == "ISA":
            ...
            point = AtmospherePoint(
                ...
                viscosity_pa_s=float(atm.dynamic_viscosity) if hasattr(atm, 'dynamic_viscosity') else None
            )
        else:
    ...
    for altitude in altitudes_m:
        ...
        if height_agl < 0:
            ...
            if model == "logarithmic":
                ...
                wind_speed = surface_wind_mps * (height_agl / reference_height_m) ** alpha

        results.append(WindPoint(
            altitude_m=altitude,
