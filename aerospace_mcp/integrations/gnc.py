...
def evaluate_trajectory_cost(trajectory: List[TrajectoryWaypoint], 
...
class GeneticAlgorithm:
    ...
    def random_trajectory(self, n_waypoints: int, constraints: OptimizationConstraints) -> List[TrajectoryWaypoint]:
        """Generate random trajectory."""
        trajectory = []
        
        for i in range(n_waypoints):
            # Random time
            t = random.uniform(constraints.time_bounds_s[0], constraints.time_bounds_s[1]) * i / (n_waypoints - 1)
            
            # Random position (around Earth orbit)
            r = random.uniform(constraints.min_altitude_m + 6.378137e6, constraints.min_altitude_m + 6.378137e6 + 1e6)
    ...
    def crossover(self, parent1: List[TrajectoryWaypoint], parent2: List[TrajectoryWaypoint]) -> List[TrajectoryWaypoint]:
    ...
    def mutate(self, trajectory: List[TrajectoryWaypoint], constraints: OptimizationConstraints) -> List[TrajectoryWaypoint]:
        """Mutate trajectory."""
        mutated = []
    ...
    def optimize(self, initial_trajectory: List[TrajectoryWaypoint],
                objective: OptimizationObjective,
        ...
        self.population = [self.random_trajectory(n_waypoints, constraints) 
        ...
        for generation in range(self.params.generations):
            # Evaluate fitness
            self.fitness_scores = [evaluate_trajectory_cost(traj, objective, constraints) 
                                 for traj in self.population]
            
            # Find best
            min_idx = min(range(len(self.fitness_scores)), key=lambda i: self.fitness_scores[i])
            ...
            sorted_indices = sorted(range(len(self.fitness_scores)), key=lambda i: self.fitness_scores[i])
            ...
            while len(new_population) < self.params.population_size:
                ...
                parent1_idx = min(random.sample(range(len(self.population)), 3), 
                                key=lambda i: self.fitness_scores[i])
                parent2_idx = min(random.sample(range(len(self.population)), 3), 
                                key=lambda i: self.fitness_scores[i])
    ...
    def optimize(self, initial_trajectory: List[TrajectoryWaypoint],
        ...
        for _ in range(self.params.num_particles):
            ...
            for _ in range(n_dimensions):
                particle.append(random.uniform(-constraints.max_thrust_n, constraints.max_thrust_n))
        ...
        for iteration in range(self.params.max_iterations):
            ...
            for i in range(len(self.particles)):
                for j in range(n_dimensions):
                    ...
                    cognitive = self.params.c1 * r1 * (self.personal_best[i][j] - self.particles[i][j])
                    social = self.params.c2 * r2 * (self.global_best[j] - self.particles[i][j])
                    
                    self.velocities[i][j] = (self.params.w * self.velocities[i][j] + 
                                           cognitive + social)
            ...
            if iteration > 10:
                ...
                if max(recent_scores) - min(recent_scores) < self.params.convergence_tolerance:
                    break
...
def monte_carlo_uncertainty_analysis(nominal_trajectory: List[TrajectoryWaypoint],
    ...
    for _ in range(n_samples):
        ...
        for waypoint in nominal_trajectory:
            # Apply uncertainties
            pos_uncertainty = uncertainty_params.get("position_m", {"std": 100.0})["std"]
            vel_uncertainty = uncertainty_params.get("velocity_ms", {"std": 10.0})["std"]
            thrust_uncertainty = uncertainty_params.get("thrust_n", {"std": 50.0})["std"]
            ...
            thrust_perturbation = [random.gauss(0, thrust_uncertainty) for _ in range(3)]
            
            perturbed_pos = [waypoint.position_m[i] + pos_perturbation[i] for i in range(3)]
            perturbed_vel = [waypoint.velocity_ms[i] + vel_perturbation[i] for i in range(3)]
            perturbed_thrust = [waypoint.thrust_n[i] + thrust_perturbation[i] for i in range(3)] if waypoint.thrust_n else None
        ...
        results["flight_time_samples"].append(perturbed_trajectory[-1].time_s - perturbed_trajectory[0].time_s)
    ...
    try:
        ...
        final_pos_errors = [vector_magnitude([
            results["final_position_samples"][i][j] - nominal_trajectory[-1].position_m[j] 
            for j in range(3)
        ...
        return {
            ...
            "confidence_intervals": {
                "delta_v_95_ms": [delta_v_mean - 1.96*delta_v_std, delta_v_mean + 1.96*delta_v_std],
                "position_95_m": [pos_error_mean - 1.96*pos_error_std, pos_error_mean + 1.96*pos_error_std]
            }
        }
