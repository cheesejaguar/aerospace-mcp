...
class UAVConfiguration(BaseModel):
    ...
    wing_area_m2: Optional[float] = Field(None, gt=0, description="Wing area for fixed-wing")
    disk_area_m2: Optional[float] = Field(None, gt=0, description="Total rotor disk area for multirotor")
    cd0: float = Field(0.03, ge=0, description="Zero-lift drag coefficient")
    cl_cruise: Optional[float] = Field(None, description="Cruise lift coefficient for fixed-wing")
...
class EnergyAnalysisResult(BaseModel):
    ...
    range_km: Optional[float] = Field(None, description="Range in km (for fixed-wing)")
    hover_time_min: Optional[float] = Field(None, description="Hover time in minutes (for multirotor)")
...
def uav_energy_estimate(
    ...
    propeller_geometry: Optional[PropellerGeometry] = None
